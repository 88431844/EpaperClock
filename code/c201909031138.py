...
#每隔半小时更新天气
        ...
        time_draw.text((10, 5), u' '+time.strftime('%m-%d')+u' 周'+digital_to_chinese(time.strftime('%w'))+u'      '+temperature+u'°C '+weather, font = font24, fill = 0)
#        time_draw.text((10, 5), time.strftime('%m-%d')+u' 周'+digital_to_chinese(time.strftime('%w')), font = font24, fill = 0)
        time_draw.text((10, 25), time.strftime('%H:%M'), font = font, fill = 0)
        time_draw.text((10,110),u'明:'+tomorrowNightTemp+u'~'+tomorrowDayTemp+u'°C '+tomorrowWeather+u' 更:'+reportTime+u'距开资:07天',font = font18,fill = 0)
